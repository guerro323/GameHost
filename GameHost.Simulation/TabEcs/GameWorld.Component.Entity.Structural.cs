    ...
    public partial class GameWorld
    {
	    ...
	    public bool RemoveMultipleComponent(GameEntityHandle entityHandle, Span<ComponentType> componentTypeSpan)
        {
            ...
            foreach (ref readonly var componentType in componentTypeSpan)
                b &= GameWorldLL.RemoveComponentReference(
                    GameWorldLL.GetComponentBoardBase(Boards.ComponentType, componentType), componentType,
                    Boards.Entity, entityHandle);
	    ...
	    public bool RemoveComponent(GameEntityHandle entityHandle, ComponentType componentType)
        {
            ...
            if (GameWorldLL.RemoveComponentReference(
                GameWorldLL.GetComponentBoardBase(Boards.ComponentType, componentType), componentType, Boards.Entity,
                entityHandle))
            {
                GameWorldLL.UpdateArchetype(Boards.Archetype, Boards.ComponentType, Boards.Entity, entityHandle);
                return true;
	    ...
	    public void AssureComponents(GameEntityHandle entityHandle, Span<ComponentType> componentTypeSpan)
        {
            ...
            foreach (ref readonly var componentType in componentTypeSpan)
            {
                ...
                if (componentType == default ||
                    entityBoard.GetComponentColumn(componentType.Id)[(int) entityHandle.Id].Valid)
                    continue;
	    ...
	    /// <param name="componentType"></param>
	    /// <returns></returns>
	    public void AddRemoveMultipleComponent(GameEntityHandle entityHandle, Span<ComponentType> addSpan,
            Span<ComponentType> removeSpan)
        {
            ThrowOnInvalidHandle(entityHandle);
            ...
            foreach (ref readonly var componentType in removeSpan)
                updateArch |= GameWorldLL.RemoveComponentReference(
                    GameWorldLL.GetComponentBoardBase(Boards.ComponentType, componentType), componentType,
                    Boards.Entity, entityHandle);
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public ComponentReference AddComponent<T>(GameEntityHandle entityHandle, ComponentType baseType,
            in T data = default)
            where T : struct, IComponentData
        {
