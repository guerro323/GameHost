...
namespace GameHost.Simulation.Utility.EntityQuery
{
	...
	public class EntityQuery : IDisposable
    {
        ...
        private readonly PooledList<uint> matchedArchetypes;
        ...
        public EntityQuery(GameWorld gameWorld, Span<ComponentType> all, Span<ComponentType> none,
            Span<ComponentType> or)
            : this(gameWorld, new FinalizedQuery {All = all, None = none, Or = or})
        {
        }

        public FinalizedQuery AsFinalized => new() {All = All, Or = Or, None = None};
        ...
        ///     Thrown when index is negative or superior/equal to
        ///     <see cref="GetEntityCount" />
        /// </exception>
        ...
        /// <remarks><see cref="CheckForNewArchetypes" /> isn't called on invocation</remarks>
        /// <remarks>
        ///     This method should be called in a while loop with it as a condition since it may not fully return all entities
        ///     in one call
        /// </remarks>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        public void AddEntitiesTo(PooledList<GameEntityHandle> list)
        {
            foreach (var archetype in matchedArchetypes.Span)
                list.AddRange(
                    MemoryMarshal.Cast<uint, GameEntityHandle>(GameWorld.Boards.Archetype.GetEntities(archetype)));
        }
        ...
        {
            foreach (var archetype in matchedArchetypes.Span)
            foreach (ref readonly var entity in MemoryMarshal.Cast<uint, GameEntityHandle>(
                GameWorld.Boards.Archetype.GetEntities(archetype)))
                list.Add(entity);
        }
        ...
        public void RemoveAllEntities()
        {
            ...
            {
                var entitySpan =
                    MemoryMarshal.Cast<uint, GameEntityHandle>(GameWorld.Boards.Archetype.GetEntities(arch));
            ...
            if (archetype >= archetypeIsValid.Length)
            {
                if (GameWorld.Boards.Archetype.Registered.Length > lastArchetypeCount)
                    throw new InvalidOperationException(
                        $"{nameof(CheckForNewArchetypes)} was not called and resulted in an archetype out of bounds in this query. (arch={archetype}, lastCount={lastArchetypeCount}, currentCount={GameWorld.Boards.Archetype.Registered.Length})");

                throw new IndexOutOfRangeException(
                    $"Archetype '{archetype}' is out of bounds from '{archetypeIsValid.Length}'");
            }
