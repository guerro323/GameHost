...
namespace GameHost.Simulation.Utility.EntitySystem
{
    ...
    public static class BatchRunnerExtensions
    {
        public static void WaitForCompletion(this IBatchRunner runner, BatchRequest request,
            bool canExecuteOnMainThread = true)
        {
    ...
    public class ThreadBatchRunner : IBatchRunner, IDisposable
    {
        ...
        private readonly BatchResult[] batchResults;

        private readonly int[] batchVersions;

        private readonly CancellationTokenSource ccs;
        private readonly ConcurrentBag<QueuedBatch> queuedBatches;
        private readonly TaskState[] states;
        ...
        public BatchRequest Queue(IBatch batch)
        {
            ...
            for (var i = 0; i < use; i++)
                queuedBatches.Add(new QueuedBatch
                {
                    BatchId = batchNumber,
                    Batch = batch,
                    Index = i,
        ...
        private static bool execute(IBatchRunner runner, QueuedBatch queued, int taskIndex, int taskCount,
            BatchResult[] results)
        {
            ...
            if (Interlocked.Increment(ref results[queued.BatchId].SuccessfulWrite) ==
                results[queued.BatchId].MaxIndex - 1
        ...
        public void StopPerformanceCriticalSection()
        {
            ...
            foreach (var state in states) state.IsPerformanceCritical = false;
        }
