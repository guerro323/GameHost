...
namespace GameHost.Simulation.Utility.EntitySystem
{
    ...
    public class ArchetypeSystem<T> : IBatch
    {
        ...
        private readonly OnArchetype action;

        private SystemState<T> currentState;

        private int entityCount;
        private readonly EntityQuery.EntityQuery query;

        public ArchetypeSystem(OnArchetype action, EntityQuery.EntityQuery query, bool forceSingleThread = false)
        {
            this.action = action;
            this.query = query;
        ...
        public void Execute(int index, int maxUseIndex, int task, int taskCount)
        {
            ...
            var prevCount = count;
            while (query.EntitySliceAt(ref start, ref count, out var span)) action(span, currentState);
        }
        ...
        {
            action(stackalloc GameEntityHandle[] {handle}, new SystemState<T> {Data = state, World = query.GameWorld});
        }
        ...
        public void Update(T state)
        {
            ...
            while (query.EntitySliceAt(ref start, ref count, out var span))
                action(span, new SystemState<T> {Data = state, World = query.GameWorld});
        }
    }
}