...
namespace GameHost.Transports
{
	...
	public partial class ThreadTransportDriver : TransportDriver
	{
		...
		private readonly IRunnableScheduler scheduler;
		...
		public override TransportConnection Accept()
		{
			...
			lock (m_Connections)
			{
				...
				{
					conSource.scheduler.Add(thisConnection =>
						{
							thisConnection.AddEvent(TransportEvent.EType.Connect);
							return true;
						},
				...
				scheduler.Add(foreignCon =>
					{
						foreignCon.AddEvent(TransportEvent.EType.Connect);
						return true;
					},
					m_Connections[id],
					default);
		...
		public override unsafe int Send(TransportChannel chan, TransportConnection con, Span<byte> data)
		{
			lock (m_Connections)
			{
				...
				var dataPtr     = NativeMemory.Alloc((uint) data.Length);
				var dataLength  = data.Length;

				data.CopyTo(new Span<byte>(dataPtr, dataLength));

				otherSource.scheduler.Add(sd =>
					{
					...
					new SendData
					{
						Connection = connection.Peer.Source.m_Connections[SelfId], 
						Data = dataPtr, 
						Length = dataLength
					},
					default);
		...
		private unsafe struct SendData : IDisposable
		{
