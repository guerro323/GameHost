...
/// <summary>
/// A module is a container that can be loaded and unloaded.
/// It can either be the entry point of an assembly, or a sub-module.
/// </summary>
public abstract partial class HostModule : IDisposable, IHasDependencies
{
    ...
    /// <summary>
    /// Return the storage from the Group without the Name
    /// </summary>
    /// <param name="scope">Context scope that must contains a <see cref="IModuleCollectionStorage"/></param>
    /// <returns>The storage</returns>
    /// <exception cref="InvalidOperationException">Parent storage couldn't be found</exception>
    protected IStorage GetGroupStorage(Scope scope)
    {
        ...
        return moduleStorage.GetSubStorage(Group);
    }

    /// <summary>
    /// Used as a one time callback.
    /// </summary>
    /// <param name="scope">Context scope that must contains a <see cref="IModuleCollectionStorage"/></param>
    /// <returns>The storage that the module will use</returns>
    protected internal virtual IStorage CreateDataStorage(Scope scope)
    {
        var storage = GetGroupStorage(scope);
