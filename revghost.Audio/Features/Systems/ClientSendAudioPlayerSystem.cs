...
public class ClientSendAudioPlayerSystem : AppSystemWithFeature<AudioClientFeature>
{
    ...
    private readonly EntitySet withoutIdSet;
    ...
    protected override void OnUpdate()
    {
        ...
        if (withoutIdSet.Count > 0)
        {
            ...
            foreach (ref readonly var entity in entities) entity.Set(new AudioPlayerId(selfLastMaxId++));
        }

        var maxId = 0;
        foreach (var entity in playerSet.GetEntities()) maxId = Math.Max(maxId, entity.Get<AudioPlayerId>().Id);
